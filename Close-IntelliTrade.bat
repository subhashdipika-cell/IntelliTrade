...
title Close IntelliTrade

powershell.exe -NoLogo -NoProfile -Command ^
  "$targets=New-Object 'System.Collections.Generic.HashSet[int]';" ^
  "$rules=@(@{Port=8100;Pattern='IntelliTrade.+uvicorn main:app'},@{Port=3001;Pattern='IntelliTrade.+next'});" ^
  ...
  "if($targets.Count -eq 0){Write-Host 'IntelliTrade is not running.'; exit 0};" ^
  "foreach($processId in $targets){Write-Host \"Stopping IntelliTrade PID $processId...\"; & taskkill.exe /PID $processId /T /F | Out-Null; if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}}; exit 0"

set "CLOSE_EXIT=%ERRORLEVEL%"
if not "%CLOSE_EXIT%"=="0" echo [ERROR] IntelliTrade could not be stopped cleanly.
